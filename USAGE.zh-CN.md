...
V0.6.2 将 Requirements draft 拆成 scope、产品设计、架构和测试策略 checkpoint，再装配成一个带 checkpoint hash 的最终 Requirements approval package。V0.6.2a 新增目标产品表面分类，确保 staged Product Design 和 Architecture 围绕目标产品/目标系统。V0.6.2b 在 Product Design 成功后启动常驻原型预览，并让该 URL 一直可用于 Requirements review。V0.6.2c 使用中文主 checkpoint 名称，并支持 Requirements checkpoint 定点 revise。V0.6.2d 增加 Unit Plan handoff 连贯性硬校验，确保下游 Builder 启动前已有 passed 的上游 `handoff-evidence.json`。V0.6.2e 将 `--spec` 扩展到真实需求文档包目录。V0.6.2f 增加 approval notes 非合同上下文、人工编辑正文受控采纳、Ctrl+C human_interrupt 恢复和 review surface 一致性证据。V0.6.2g 增加无 spec Product Design brainstorming prompt 分支，并保持 annotation pass 只走 subprocess runtime。V0.6.2h 将 Requirements 4.6 E2E parsing 限定到 canonical 固定列矩阵块，后续 subsection 表不会变成 4.6 obligations。V0.6.2i 是 prompt-only 版本，补充无 spec 验收前置 intake、1:1 用户任务原型和 Product Journey Contract 交接文案。`WAYGATE_ANNOTATION_TMUX` 已废弃并被忽略。
...
# 从受支持的 requirements spec 文件或文档包目录启动。
...
`--spec <path>` 可以导入可读的本地 Waygate Markdown spec 文件、受支持 OpenSpec/OpenAPI 文档、Open Spec package directory 或 Spec Kit feature package directory。Waygate 会在 `session.json` 中保存 path、SHA-256 hash、source type、import time 和 conversion artifact path，不保存 spec 全文。Open Spec package 目录必须包含 `01-requirements.md`，并至少包含 `02-specification.md`、`03-technical-solution.md`、`04-storage-design.md` 或 `08-stage-handoff.md` 之一。Spec Kit feature package 目录必须包含 `spec.md`，并有 `plan.md`、`tasks.md`、`research.md`、`data-model.md`、`quickstart.md` 或 `contracts/`；请传入 `specs/<feature>/` 或具体 `spec.md`，不要传 `.specify` 工具/工作区根目录。
...
支持的 role alias 是 `requirements`、`unit-plan`、`final-acceptance` 和 `all`。支持的 backend 只有 `codex` 和 `opencode`。`--annotation-agent claude`、`--annotation-agent claude-code` 及 role 形式会被拒绝；Claude Code 仍可通过 `tmux-claude` 作为普通 workflow runner 使用。Annotation subprocess 会在父进程存在时默认继承标准代理 key：`HTTP_PROXY`、`HTTPS_PROXY`、`ALL_PROXY`、`NO_PROXY` 及对应小写形式。可用 `--no-annotation-agent ROLE|all` 禁用 role，`--annotation-agent-env-key ROLE=KEY` 继承额外非代理环境变量名，`--annotation-agent-timeout ROLE=SECONDS` 设置超时，`--annotation-agent-failure-policy ROLE=block|warn` 设置失败策略。

内置 `--annotation-agent codex` 会启用三个 role，使用 `command=codex`、`args=["exec", "--sandbox", "workspace-write", "-o", "{artifact_path}", "..."]`、`timeout_seconds=7200`、`failure_policy=block` 和 `prompt_template=risk-json-v1`。内置 `--annotation-agent opencode` 使用 `command=opencode`、`args=["run", "..."]`。Annotation 输出只能辅助标注风险，不能批准、跳过、修改或绕过任何 Waygate gate。Waygate 旧内置 Codex annotation args 会自动归一化；已有 session 中的 Waygate 内置 Claude annotation 配置会迁移成内置 OpenCode 模板。自定义 `--annotation-agent-cmd` 不会被改写，但声明 backend 必须仍是 `codex` 或 `opencode`。

Annotation Agent 始终通过 subprocess runtime 执行配置里的 command/args。旧 shell 中如果仍存在 `WAYGATE_ANNOTATION_TMUX`，它会作为废弃 no-op 被接受但忽略；不会创建 pane、run-local wrapper、annotation run 目录、`done.json` 或 tmux fallback event。runtime metadata 只做 env key-only 审计，记录 command、args、prompt path、artifact path、backend、timeout 和 env key 名，不记录 env value。

进入人工 gate 前，controller pane 会打印 `标注 Agent 开始：角色=requirements_annotation 后端=codex 产物=<path>`、`标注 Agent 完成：角色=requirements_annotation 返回码=0 用时=<duration>` 或 `标注 Agent 失败：角色=requirements_annotation 错误=<summary> 产物=<path> 用时=<duration>` 这类紧凑中文生命周期行；`--color always` 和 TTY 下的 `--color auto` 会给 Agent 标签与状态上色，`--color never` 保持纯文本。stdout/stderr 仍只写入 artifact 和 event。当 fresh annotation artifact 与当前 gate body 匹配后，人工 gate 菜单会显示 artifact 路径、风险数量和中文紧凑摘要，Plannotator review metadata 也会记录同一个 artifact 引用。人类可见批注字段必须是简体中文；英文-only 的 `summary` 或风险 message 会被拒绝，不会作为当前批注展示。Requirements 修订会重新运行 Requirements annotation，artifact 会记录 `gate_content_hash` 和 `human_language=zh-CN`，因此旧 gate body 或旧 prompt 合同的 annotation 不会被当作当前审阅上下文复用。

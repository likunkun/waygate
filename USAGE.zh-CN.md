...
Waygate 默认把 Plannotator 审批页显示为 `http://0.0.0.0:<port>`，把原型预览页显示为 `http://0.0.0.0:<preview-port>/plannotator-review.html`。`0.0.0.0` 是监听/展示地址；用远程浏览器打开时，通常需要替换成运行 Waygate 的机器 IP 或主机名。`WAYGATE_PREVIEW_HOST` 可以覆盖 controller preview host；Waygate 默认向 Plannotator 传入 `PLANNOTATOR_HOST=0.0.0.0`。该变量是否能改变 Plannotator 内部 bind 行为，取决于 Plannotator 本体。

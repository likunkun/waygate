...
| Staged Requirements Package | V0.6.2 splits overloaded Requirements drafting into focused scope, product design, architecture, and test strategy checkpoints; V0.6.2a keeps those checkpoints anchored to the target product/system surface; V0.6.2b keeps the Product Design prototype preview available through Requirements review; V0.6.2c uses Chinese-primary checkpoint names and supports targeted Requirements checkpoint revise; V0.6.2d adds hard multi-unit handoff continuity evidence before downstream Builder execution; V0.6.2e accepts real requirements package directories through `--spec`; V0.6.2f adds approval-notes advisory context, guarded manual gate adoption, Ctrl+C human interruption recovery, and review-surface conformance evidence; V0.6.2g adds no-spec Product Design brainstorming prompt branches and keeps annotation passes subprocess-only; V0.6.2h keeps 4.6 E2E matrix validation scoped to the canonical fixed-column table block; V0.6.2i adds prompt-only Requirements acceptance-first intake, 1:1 user-task prototypes, and Product Journey Contract handoff language. |
| External spec intake | V0.6.1 imports supported OpenSpec/OpenAPI and Spec Kit sources into auditable conversion artifacts; V0.6.2e also imports Open Spec package directories and Spec Kit feature package directories while preserving clear unsupported/deferred errors. |
| Unit planning gates | Unit plans must map objectives, acceptance criteria, test cases, journeys, and verification commands. |
| Annotation agents | V0.6.1 can run non-approving role-based annotation and verification-assist passes before human gates for Requirements, Unit Plan, and Final Acceptance. Annotation runs are subprocess-only and support `opencode` or `codex` as declared backends. Claude Code remains available as a normal workflow runner through `tmux-claude`, but it is not an annotation agent backend. |

...
- 澄清问题必须先确认当前版本目标、非目标、验收重点、成功/失败证据和范围边界；不能在用户回答前直接缩小需求范围或把后续 backlog 当作当前版本范围。
...
- Requirements 阶段必须形成可继承的 Product Journey Contract：用真实用户任务描述 actor、任务起点、主业务对象、关键状态/事件、成功终点、对应 AC/Journey 和生产目标；产品旅程闭环不能由 fixture、工程层、截图或 prototype artifact 单独替代。
...
- `surface_contracts` 用于拆分原型中的每个可交互 UI surface，兼容别名 `ui_surfaces` / `page_state_targets`。每个 required surface 必须包含 `id`, `title`, `kind`（`route | page | component | dialog | drawer | panel | form | other`）, `actor`, `task_start`, `main_business_object`, `success_endpoint`, `page_states[]`, `click_path[]`, `entrypoints[]`, `implementation_targets[]`, `linked_acceptance_criteria[]`, `linked_journeys[]`, `required: true`。Scope/Product Design 中声明的 visible surface 必须全部登记到 manifest。

...
def _make_fake_tmux_that_completes_requirements_scope(
    tmp_path: Path,
    *,
    split_pane_id: str = '%89',
    stale_targets: list[str] | None = None,
) -> Path:
    tmux_log = tmp_path / 'tmux.log'
    env_snapshot = tmp_path / 'tmux-env.json'
    fake_tmux = tmp_path / 'tmux'
    stale_targets = stale_targets or []
    fake_tmux.write_text(
        f"""#!/usr/bin/env python3
...
if args[:2] == ["display-message", "-p"]:
    ...
    run_dir = Path(os.environ["RRC_RUN_DIR"])
    (run_dir.parent.parent / "requirements-scope.md").write_text(
        "# Requirements Scope Checkpoint\\n\\n- Generated by fake tmux.\\n",
        encoding="utf-8",
    )
    Path({str(env_snapshot)!r}).write_text(
        json.dumps({{
            "WAYGATE_TMUX_CLEAR_INPUT_BEFORE_DISPATCH": os.environ.get("WAYGATE_TMUX_CLEAR_INPUT_BEFORE_DISPATCH"),
            "RRC_TMUX_CLAUDE_SUBMIT_DELAY_SECONDS": os.environ.get("RRC_TMUX_CLAUDE_SUBMIT_DELAY_SECONDS"),
            "WAYGATE_TMUX_CLAUDE_SUBMIT_WATCHDOG": os.environ.get("WAYGATE_TMUX_CLAUDE_SUBMIT_WATCHDOG"),
        }}, sort_keys=True),
        encoding="utf-8",
    )
    Path(os.environ["RRC_RUN_DONE_FILE"]).write_text(
        json.dumps({{
            "status": "done",
            "summary": "requirements scope generated",
...
def test_unit_plan_approval_rejects_assist_only_candidate_after_human_approval(tmp_path: Path) -> None:
    state_dir = tmp_path / 'state'
    controller = _init_unit_plan_preflight_controller(
        state_dir,
        {
...
def test_unit_plan_approval_rejects_missing_infrastructure_matrix_after_human_approval(tmp_path: Path) -> None:
    state_dir = tmp_path / 'state'
    controller = _init_unit_plan_preflight_controller(
        state_dir,
        {
    ...
    assert state['currentStep'] == 'WAITING_UNIT_PLAN_APPROVAL'
    assert state['unitPlanAccepted'] is False
    assert 'Infrastructure / Execution Context Matrix' in state['blockedReason']
...
def test_rrc_go_dry_run_target_starts_with_requirements_scope_checkpoint(tmp_path: Path) -> None:
    workspace = tmp_path / 'workspace'
    workspace.mkdir()

    result = run_rrc_with_pythonpath('go', 'V1.0', '--runner', 'subprocess', '--dry-run', '--max-steps', '1', cwd=workspace)
    ...
    assert state['stagedRequirementsEnabled'] is True
    assert state['requirementsPackage']['version'] == 'v0.6.2-staged'
    assert (state_dir / 'artifacts' / 'requirements-scope' / 'requirements-scope.md').exists()
    assert not (state_dir / 'artifacts' / 'requirements-draft' / 'requirements-body.md').exists()
...
def test_rrc_go_recreated_staged_scope_pane_dispatch_disables_initial_clear(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    workspace = tmp_path / 'workspace'
    ...
    state_path = workspace / '.rrc-controller-v1.0' / 'session.json'
    state = json.loads(state_path.read_text(encoding='utf-8'))
    assert state['currentStep'] == 'REQUIREMENTS_SCOPE_DRAFT'
    assert state['requirementsPackage'] == {'version': 'v0.6.2-staged', 'artifacts': {}}
    assert state['tmuxTarget'] == '%88'
    assert state['tmuxTargetResolution']['source'] == 'auto-created'

    _make_fake_tmux_that_completes_requirements_scope(
        tmp_path,
        split_pane_id='%89',
        stale_targets=['%88'],
    )
    ...
    assert state['tmuxTargetResolution']['source'] == 'auto-created'
    assert state['currentStep'] == 'REQUIREMENTS_PRODUCT_DESIGN_BRIEF'
    assert state['requirementsPackage']['artifacts']['scope']['status'] == 'complete'

    env_snapshot = json.loads((tmp_path / 'tmux-env.json').read_text(encoding='utf-8'))
    assert env_snapshot == {
    ...
    assert ['send-keys', '-t', '%89', 'C-c'] not in tmux_calls
    assert ['send-keys', '-t', '%89', 'C-u'] not in tmux_calls
    assert ['paste-buffer', '-t', '%89'] in tmux_calls
    assert ['send-keys', '-t', '%89', 'C-m'] in tmux_calls
...
def test_plannotator_close_keeps_requirements_prototype_preview_server_running(
    tmp_path: Path,
    monkeypatch,
) -> None:
    state_dir = tmp_path / 'state'
    ...
    html_review_path.write_text('<!doctype html><p>Prototype review</p>\n', encoding='utf-8')
    manifest_path.write_text('{"version":"v0.6.0b"}\n', encoding='utf-8')
    fake_summary_path = state_dir / 'plannotator' / 'requirements-last-review.json'
    fake_summary_path.parent.mkdir(parents=True)
    fake_summary_path.write_text('{"gate_path":"requirements"}\n', encoding='utf-8')

    class FakePreviewServer:
        preview_url = 'http://192.0.2.55:20001/plannotator-review.html'
        base_url = 'http://192.0.2.55:20001'
        port = 20001
        close_count = 0
    ...
    gate_info = {
        ...
        'can_revise': True,
        'can_rework': False,
        'prototype_review_path': html_review_path,
        'prototype_review_manifest_path': manifest_path,
        'prototype_review_prototypes_dir': prototypes_dir,
    }

    handled = controller._handle_drive_gate(
        gate_info,
        'tester',
    ...
    assert fake_server.close_count == 0
    assert any('原型渲染预览页: http://192.0.2.55:20001/plannotator-review.html' in line for line in output)
    getattr(controller, 'close', lambda: None)()


def test_pending_requirements_gate_lazily_generates_prototype_review_bundle(
    tmp_path: Path,
) -> None:
    state_dir = tmp_path / 'state'
    controller = RalphRefinerController(state_dir=state_dir, auto_approve=True)
    ...
    approval_path.write_text(
        '# Requirements & Acceptance Confirmation\n\n'
        '## Acceptance Criteria\n'
        '- AC-01 [verification: functional]: Delivery evidence is visible.\n\n'
        '## Human Confirmation\n\n'
        'Status: pending\n',
        encoding='utf-8',
    )
    draft_dir = state_dir / 'artifacts' / 'requirements-draft'
    prototype_dir = draft_dir / 'prototypes' / 'delivery-flow'
    prototype_dir.mkdir(parents=True)
    (prototype_dir / 'index.html').write_text(
        '<!doctype html><button>Deliver</button>\n',
        encoding='utf-8',
    )
                ...
                'prototypes': [
                    {
                        'id': 'delivery-flow',
                        'type': 'html',
                        'path': 'prototypes/delivery-flow/index.html',
                        'title': 'Delivery flow',
                        'linked_acceptance_criteria': ['AC-01'],
                        'page_states': ['Entry', 'Submitted'],
                        'click_path': ['Open delivery', 'Submit delivery'],
                    }
                ]
            }
        ),
    ...
    assert not (draft_dir / 'plannotator-review.html').exists()
    assert not (draft_dir / 'prototype-review-manifest.json').exists()

    gate_info = controller._pending_gate_info(controller.store.load_state())

    assert gate_info is not None
    assert gate_info['prototype_review_path'] == draft_dir / 'plannotator-review.html'
    assert gate_info['prototype_review_manifest_path'] == draft_dir / 'prototype-review-manifest.json'
    assert gate_info['prototype_review_prototypes_dir'] == draft_dir / 'prototypes'
...
def test_blocked_guidance_shows_category_and_compacts_long_blocker_lists(tmp_path: Path) -> None:
    state_dir = tmp_path / 'state'
    blockers = [
        f'approved acceptance criterion AC-{index} has no passed or valid manual evidence row'
    ...
    state = {
        ...
        'blockedContext': {
            'category': 'unit_plan_contract',
            'artifact': 'artifacts/final-scope-audit/scope-audit.json',
        },
    }
    ...
    assert '类别：unit_plan_contract' in guidance
    assert blockers[0] in guidance
    assert blockers[2] in guidance
    assert blockers[3] not in guidance
    assert '完整 blocker 列表' in guidance
    assert 'artifacts/final-scope-audit/scope-audit.json' in guidance
    assert f'waygate revise --gate unit-plan --state-dir {shlex.quote(str(state_dir))}' in guidance
...
def test_requirements_auto_revision_state_clears_after_valid_preflight(tmp_path: Path, monkeypatch) -> None:
    from workflow_controller.gates.parsers import write_gate_file
    ...
    controller.init_state(
        {
            ...
            'tmuxTarget': '1.2',
            'requirementsAutoRevisionMax': 2,
            'requirementsAutoRevisionLastReasonKey': 'scope:e2e_mapping',
            'requirementsAutoRevisionConsecutiveCount': 2,
            'requirementsAutoRevisionTotalCount': 5,
            'humanGatesRequired': True,
            'requirementsAccepted': False,
            'requirementsDraftGenerated': True,
            'objectiveCoverage': [
    ...
    monkeypatch.setattr(
        controller,
        '_requirements_gate_invalid_reason',
        lambda _state, _gate_path: None,
    )

    state = controller._auto_revise_invalid_requirements_draft(controller.store.load_state())

    assert state.get('blockedReason') is None
    assert 'requirementsAutoRevisionLastReasonKey' not in state
    assert 'requirementsAutoRevisionConsecutiveCount' not in state
    assert 'requirementsAutoRevisionTotalCount' not in state
...
def test_requirements_auto_revision_budget_still_blocks_repeated_same_reason(tmp_path: Path, monkeypatch) -> None:
    ...
    assert state['blockedContext']['category'] == 'requirements_contract'
...
def test_final_acceptance_rejection_does_not_turn_generated_gate_body_into_obligations(tmp_path: Path) -> None:
    state_dir = tmp_path / 'state'
    controller = RalphRefinerController(state_dir=state_dir, auto_approve=True)
    controller.init_state(
        {
            ...
            'acceptanceObligations': [
                {
                    ...
                    'title': 'original obligation',
                    'description': 'original obligation',
                    'priority': 'must',
                    'status': 'open',
                }
            ],
    ...
    gate_path.write_text(
        ...
        '- 验证：passed\n\n'
        '## 证据文件\n'
        '- `/tmp/artifacts/unit-01/verification.json`\n'
        '- `/tmp/artifacts/unit-01/green-test.txt`\n\n'
        '## 人工审阅清单\n'
        '- [ ] 实际结果满足已批准的验收标准。\n'
        '- [ ] 已知问题已接受或记录。\n\n'
        '## 修改清单\n\n'
    ...
    assert state['currentStep'] == 'EXECUTE_UNIT'
    assert state['finalAcceptanceRejectionRoute'] == 'implementation'
    assert state['finalAcceptanceRejectionFeedback'].startswith('# 最终验收确认')
    assert [item['id'] for item in state['acceptanceObligations']] == ['AO-001']


def test_final_acceptance_rejection_ignores_default_rejection_notes_instruction(tmp_path: Path) -> None:
    state_dir = tmp_path / 'state'
    controller = RalphRefinerController(state_dir=state_dir, auto_approve=True)
    controller.init_state(
        {
    ...
    state = json.loads((state_dir / 'session.json').read_text(encoding='utf-8'))
    assert state['currentStep'] == 'EXECUTE_UNIT'
    assert state.get('acceptanceObligations', []) == []


def test_status_closes_legacy_generated_final_rejection_obligations(tmp_path: Path) -> None:
    state_dir = tmp_path / 'state'
    controller = RalphRefinerController(state_dir=state_dir, auto_approve=True)
    controller.init_state(
        {
            ...
            'blockedReason': (
                'final acceptance gate invalid: final scope audit has blocker(s): '
                'active must AO AO-002 has no passed or valid manual evidence row'
            ),
            'requestedOutcome': 'usable-system',
            'feasibleOutcome': 'usable-system',
            'scopeApproved': True,
            ...
            'acceptanceObligations': [
                ...
                },
                {
                    ...
                    'title': '当前阶段：`FINAL_WALKTHROUGH_PREPARE`',
                    'description': '当前阶段：`FINAL_WALKTHROUGH_PREPARE`',
                    'priority': 'must',
                    'status': 'open',
                },
                {
                    ...
                    'source': 'final_acceptance_rejection',
                    'sourceRef': 'final-acceptance:rejection-1',
                    'title': 'homepage logo is still text-only',
                    'description': 'homepage logo is still text-only',
                    'priority': 'must',
                    'status': 'open',
                },
                {
                    ...
                    'source': 'final_acceptance_rejection',
                    'sourceRef': 'final-acceptance:rejection-1',
                    'title': 'AO coverage: `7/7`',
                    'description': 'AO coverage: `7/7`',
                    'priority': 'must',
                    'status': 'open',
                },
                {
                    ...
                    'source': 'final_acceptance_rejection',
                    'sourceRef': 'final-acceptance:rejection-1',
                    'title': 'Review target-v0-4 evidence and confirm fixed PDF fixture source path.',
                    'description': 'Review target-v0-4 evidence and confirm fixed PDF fixture source path.',
                    'priority': 'must',
                    'status': 'open',
                },
            ],
    ...
    state = json.loads((state_dir / 'session.json').read_text(encoding='utf-8'))
    obligations = {item['id']: item for item in state['acceptanceObligations']}
    assert obligations['AO-001']['status'] == 'open'
    assert obligations['AO-002']['status'] == 'out_of_scope'
    assert 'generated Final Acceptance gate content' in obligations['AO-002']['closureReason']
    assert obligations['AO-003']['status'] == 'open'
    assert obligations['AO-004']['status'] == 'out_of_scope'
    assert obligations['AO-005']['status'] == 'out_of_scope'

...
- [x] tmux-codex 清输入自退出修复：Codex dispatch 前默认只发送 `C-u`，不再复用 Claude 的 `C-c`，避免清理输入框时中断/退出 Codex TUI。
- [x] V0.5.6 Final Acceptance 已批准：完成 Spec Intake & Dependency Documentation，目标 `Complete V0.5.6 development acceptance using current planning progress` 已 covered，单元 `v0-5-6-u1-spec-intake-dependency-docs` 已 passes=true。
- [x] V0.6.0 Final Acceptance 已批准：完成 Infrastructure Knowledge Base / infrastructure intake gate，目标 `Complete V0.6.0 development acceptance using current planning progress` 已 covered，单元 `v0-6-0-u1-infrastructure-intake-gate` 已 passes=true。
- [x] V0.6.0a Prototype Review Bundle 已实施：Requirements prototype manifest 规范化、Plannotator review bundle、只读 localhost preview、approval gate 分离和 UI/UX/Web prototype preflight 已落地；基线提交为 `dc3cd0b`，全量测试 `407 passed`，已生成 `dist/waygate_0.6.0a_all.deb`。
...
- [x] Requirements staged checkpoint validation 前移：Scope/Product Design/Architecture/Test Strategy checkpoint 生成后立即校验 canonical E2E/browser/prototype 映射、Scope-owned AC/Journey 引用、固定 4.6 E2E matrix，并确认 Unit Plan draft preflight 与 approval revalidation 共用同一 gate helper。
- [x] Requirements 4.6 / Unit Plan 职责边界修复：4.6 `Verification Command` 改为 command intent / command family / runner intent，Journey row 可覆盖映射 AC，prototype-only artifact review 不再触发 real E2E command strictness，exact command / test case / fixture script / evidence row 归 Unit Plan。
- [x] Requirements Journey 表头与 support layer 解析修复：Journey contract 和 4.6 E2E Journey 收集接受 `Journey id`、`User steps`、`Linked AC` 等表头别名，并允许 active support/baseline/prerequisite Journey 使用 `static`、`regression`、`prerequisite` 非 E2E layer。
- [x] Annotation timeout bytes 输出序列化修复：annotation / verification-assist timeout 输出先归一化为文本再写 JSON artifact，`failure_policy=warn` 的 Requirements annotation timeout 不再阻断已通过 deterministic preflight 的 human gate；Classroom V0.4 已推进到 Unit Plan draft。
- [x] Unit Plan annotation 中断恢复修复：Unit Plan gate preflight 通过后先持久化 `WAITING_UNIT_PLAN_APPROVAL` 和 pending annotation context；若重启时 state 仍卡在 `UNIT_PLAN_DRAFT` 但已有有效 Unit Plan gate，则恢复 draft body/summary 并接回 human gate，不重派 drafter 或删除 body。
...
- [x] Controller Prototype Fidelity Gate：UI/Web prototype conformance 默认强制 L1 visual evidence + L2 structural/interaction，Verifier 记录 `visual_evidence_refs`，Final Acceptance 展示并阻断缺截图、缺 action path、交互截图缺失、遮挡和显式 L3/L4 缺截图回归结果。
- [x] V0.6.0k UI/UX Skill Policy：Requirements、Unit Plan、Builder 和 UI Design Brief 明确 UI/Web/prototype 工作必须使用 `ui-ux-pro-max`；`frontend-design` 仅可作为全新视觉探索或局部润色辅助；doctor 对只安装 `frontend-design` 输出 warning，并优先匹配 `ui-ux-pro-max`。
- [x] V0.6.0m Golden Path E2E 前置校验：Unit Plan approval 阻断非 `layer=e2e`、非真实环境、缺真实入口、缺 fixture/setup、命令未进入 `verification_commands` 或 mock 核心 API 的 `golden_path: true`；Requirements E2E AC / active E2E Journey 必须映射 `layer=e2e` test case；API-only/service-only E2E 不要求浏览器。
- [x] V0.6.1 Final Acceptance 已批准并已打包：完成 OpenSpec / Spec Kit intake、gate ordering、role-based annotation / verification-assist、prompt contract、flexible evidence、正式 workflow / architecture 文档和全量回归闭环；目标 `Complete V0.6.1 development acceptance using current planning progress` 已 covered，单元 `v0-6-1-u1-external-spec-intake`、`v0-6-1-u2-annotation-config-prompts`、`v0-6-1-u3-flexible-evidence`、`v0-6-1-u4-docs-regression` 已 passes=true；已生成 `dist/waygate_0.6.1_all.deb`。
...
- [x] V0.6.2e `--spec` 文档包目录 intake：新增 `sourceType=open-spec-package`，支持 Open Spec package directory 和任意名称 Spec Kit feature package directory；`.specify` workspace/tool root 与普通 docs 目录明确拒绝；生成 package conversion artifacts，版本更新为 `0.6.2e` 并重新打包。
- [x] V0.6.2f Final Acceptance 已批准：完成 Human Review Control and Interruption Recovery；目标 `Complete V0.6.2f development acceptance using current planning progress` 已 covered，单元 `target-v0-6-2f` 已 passes=true；正式 workflow / architecture 文档、review bundle / prototype conformance evidence 和 focused/full regression 均已完成。
- [x] V0.6.2g Product Design Prompt Contract 与 Annotation subprocess-only 收敛已实施：Product Design prompt 增加 no-spec brainstorming、supported-spec compatibility 和 backend/API/CLI-only no-UI confirmation 分支；annotation 专用 tmux runtime 已移除，`WAYGATE_ANNOTATION_TMUX` 作为废弃 no-op 被忽略，不再创建临时 annotation pane、wrapper、run id 或 `done.json`；annotation backend 仅支持 `opencode` / `codex`，内置 Claude annotation 配置迁移到 OpenCode，普通 `tmux-claude` / `tmux-codex` workflow runner 能力保留；正式文档、script-entry verification 和版本记录已同步。
- [x] Requirements source-label parser 与 AC/Journey routing 修复：AC/Journey ID 不再从 `AC-V10-*` / `AC-V10-` 解析出伪 ID；staged Requirements 当前 AC 收集忽略 `Source AC` / `Source AC / TC` provenance 列；Journey-local `[verification: manual]` 不再扩散到同一行 AC；conflicting AC verification layers 路由到 Scope；全量 `workflow_controller/tests` 通过 `874 passed`。

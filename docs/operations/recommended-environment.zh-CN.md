...
本文说明 Waygate V0.6.0h 推荐的本地运行环境。它是给工作站或类 CI 打包验证环境使用的运维说明，不是新的需求 intake 格式。

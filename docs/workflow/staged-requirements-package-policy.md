...
Source-provenance columns such as `Source AC` and `Source AC / TC` are not current-version AC obligations. They may contain imported IDs or wildcard examples such as `AC-SPEC-*`, but those labels are ignored for missing-layer and current AC coverage checks unless the same ID is declared in a canonical current AC column or AC line. AC and Journey ID parsing also rejects trailing-hyphen fragments such as `AC-V10-` so wildcard examples cannot become partial active IDs.

...
This document records the V0.6.2 architecture boundaries for Staged Requirements Package, including the V0.6.2a target-product perspective patch, V0.6.2b persistent prototype preview patch, V0.6.2c Chinese checkpoint / targeted revise patch, V0.6.2d unit continuity handoff gate, V0.6.2e package-directory intake extension, V0.6.2f human review control handoff, V0.6.2g Product Design prompt branching, and V0.6.2i Product Journey Contract prompt handoff. It covers package helpers, target surface classification, checkpoint prompts, stage runner, controller orchestration, controller process-level prototype preview, final gate assembly, validators, Unit Plan prompt inheritance, unit handoff evidence, review-control prompt handoff, journey-contract prompt text, and revision routing.
...
| Prompt rendering | `workflow_controller/prompts/requirements.py`, `workflow_controller/prompts/requirements_package.py` | Renders legacy Requirements and staged checkpoint prompts with acceptance-first no-spec intake, Product Design 1:1 user-task prototype language, Product Journey Contract language, and upstream artifact path/hash inputs. |
...
| Unit Plan handoff | `workflow_controller/prompts/unit_plan.py` | Injects staged package artifact path/hash/status metadata into the Unit Plan prompt and requires inherited AC, Journey, design, architecture, test strategy, E2E, UI, risk obligations, Product Journey Contract facts, and `主业务对象血缘拆分矩阵` main business object lineage. |
...
| Journey-contract prompt consumers | `workflow_controller/prompts/unit_plan.py`, `workflow_controller/prompts/builder.py`, `workflow_controller/steps/builder.py` | Carry Product Journey Contract language into Unit Plan, Builder, Test Strategist, and Refiner prompts. This is prompt text only; no deterministic validator, state schema field, CLI option, manifest hard requirement, or hard gate is added in V0.6.2i. |
...
V0.6.2i extends `render_product_design_prompt()` with 1:1 user-task prototype guidance. The prompt asks each prototype or surface to name actor, task start, click path, page states, main business object, success endpoint, AC/Journey mapping, and production target. The manifest example may include prompt-level Product Journey fields such as `actor`, `task_start`, `main_business_object`, and `success_endpoint`, but `validate_staged_requirements_stage_output()` does not turn those fields into required manifest keys in this release.
...
V0.6.2i adds Product Journey Contract inheritance to the prompt only. Unit Plan must render `## 主业务对象血缘拆分矩阵` and decompose main business object lineage as Actor -> 用户任务 -> 主业务对象 -> 起点 -> 状态/事件 -> 成功终点 -> AC/Journey -> Production Target -> Unit -> Test Case/Evidence. Builder and Refiner prompts preserve the same contract so implementation and cleanup do not replace journey closure with fixtures, engineering layers, screenshots, or prototype artifacts.
